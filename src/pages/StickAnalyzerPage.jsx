    ...
    <div className="fade-in max-w-4xl">
      ...
      {result.flightStyle && (
        <div className="card mb-4">
          <span className="text-xs text-gray-400">{t('label_detected_style')}: </span>
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-gray-400">{t('label_smoothness')}</span>
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-gray-400">{t('label_symmetry')}</span>
                    <span className="text-gray-300">{((d.symmetry ?? 0) * 100).toFixed(0)}%</span>
                  </div>
                  <ProgressBar value={(d.symmetry || 0) * 100} color="green" />
                </div>
                <div className="text-xs text-gray-400">
                  {t('label_bounceback')}: <span className={`${d.bounceback > 0.3 ? 'text-red-400' : 'text-green-400'}`}>{(d.bounceback ?? 0).toFixed(2)}</span>
                </div>
                <div className="text-xs text-gray-400">
                  {t('label_jitter')}: <span className={`${d.jitter > 5 ? 'text-yellow-400' : 'text-green-400'}`}>{d.jitter?.toFixed(1) ?? '0'}</span>
                </div>
                <div className="text-xs text-gray-400">
                  {t('label_center_usage')}: <span className="text-gray-300">{((d.centerUsage ?? 0) * 100).toFixed(0)}%</span>
                </div>
              </div>
            </div>
          );
